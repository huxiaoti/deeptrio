# -*- coding: utf-8 -*-
"""
Created on Mon Feb 22 14:29:56 2021
    ...
    while line != '':
        if line.startswith('>'):
            ...
            if flo == '*':
                ...
                if float(number) > 90 and ('reviewed' not in pro_name) and count == 0:              
...
with open('ind_train_random.tsv', 'w') as w1:
    
    for key in list(pro_pair_r.keys()):
        w1.write(key + '\t' + pro_pair_r[key] + '\n')
        
    with open('ind_test_shuffle.tsv', 'w') as w2:
        pro_negative = []
        for key in list(pro_pair_s.keys()):
            name_1 = key.split('\t')[0]
            name_2 = key.split('\t')[1]
            if (name_1 not in pro_r_names) and (name_2 not in pro_r_names):
                if pro_pair_s[key] == '0':
                    rd = np.random.randint(1,2865)
                    if rd <= 846:
                        pro_negative.append(key)
#                        w2.write(key + '\t' + pro_pair_s[key] + '\n')
                elif pro_pair_s[key] == '1':
                    w2.write(key + '\t' + pro_pair_s[key] + '\n')
                else:
                    print('error')
#        remove_num = np.random.randint(0,869,size=[24,])
#        for n in remove_num:
#            pro_negative.pop(n)
